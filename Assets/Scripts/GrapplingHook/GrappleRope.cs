...
public class GrappleRope : MonoBehaviour
{
    ...
    private void LinePointsToFirePoint()
    {
        ...
        {
            m_lineRenderer.SetPosition(i, grappleGun.firePoint.position);
        }
    }
    ...
    void DrawRope()
    {
        if (!strightLine)
        {
            if (m_lineRenderer.GetPosition(percision - 1).x == grappleGun.grapplePoint.x)
            {
                strightLine = true;
            }
            else
    ...
    {
        m_lineRenderer.SetPosition(0, grappleGun.firePoint.position);
        m_lineRenderer.SetPosition(1, grappleGun.grapplePoint);
    }
}
